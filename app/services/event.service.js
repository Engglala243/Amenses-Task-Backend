...
export async function createEvent({
  ...
  const uniqueParticipantIds = Array.from(
    new Set([creatorId, ...(participants || [])])
...
export async function getEventById({ id, userId }) {
  ...
  if (
    ...
    !event.participants.map(String).includes(userId)
  ) {
...
export async function ensureCreator(eventId, userId) {
  ...
  if (event.creator.toString() !== userId)
    throw createError(403, "Only creator can modify the event");
  return event;
}
...
export async function listInvitations({ userId }) {
  return Invitation.find({ toUser: userId }).populate(
    "event",
    "title description"
  );
}
...
export async function respondInvitation({ invitationId, userId, action }) {
  ...
  if (inv.toUser.toString() !== userId.toString()) throw createError(403, "Forbidden");
  inv.status = action === "accept" ? "accepted" : "declined";
  await inv.save();
  if (inv.status === "accepted") {
    await Event.updateOne(
      { _id: inv.event, participants: { $ne: userId } },
...
export async function voteOnEvent({ id, userId, optionId }) {
  ...
  const optionExists = event.poll.options.some(
    (o) => o._id.toString() === optionId
  );
  if (!optionExists) throw createError(400, "Invalid poll option");
  event.poll.votes = event.poll.votes.filter(
